#' Before the current analysis was finalized, we needed to explore whether duplicate
#' ids appeared in multiple pairs. They did. This file is now irrelevant due to the
#' introduction of `005_make_id_crosswalk.R`, but is preserved for posterity.
...
dat0 <- read_csv(here("data", "MERGE_NR_2.3.21.csv"))
...
## Checking that dec_dat$caseid is the same as dat1$caseid for survey 8 so that
## we can connect the two datasets on at least one id
...
## This loads other stuff, but we don't need it. Probably nicer to just save a
## copy of dat3 elsewhere, recorded in the Makefile, etc.. For now, continue on.
## verbose=TRUE here to remind us of the stuff.
...
dec_dat1 <- dec_dat %>%
  select(contains("caseid")) %>%
  mutate(caseid_16 = caseid)
dat3_thin <- dat3 %>% select(orig_id, bm, survey)

## Verify that all matches were within survey:
same_survey_dat_verify <- dat3_thin %>%
  group_by(bm) %>%
  summarize(same_survey = length(unique(survey)))
...
## For sanity, make sure that exactly one of the columns above is TRUE
stopifnot(all(dat3_thin %>% select(starts_with("orig_id_is_caseid")) %>% rowSums() <= 1))

## So, orig_id 1264497677 matches one of the caseid_15 numbers
dat3_thin
...
## How often to we have an orig_id which doesn't match any id in the id
## crosswalk csv file? Answer: 636 people
...
dat3_thin$which_caseid <- with(
  dat3_thin,
  case_when(
    orig_id_is_caseid_16 ~ 16,
...
dat4 <- left_join(dat3_thin,
  ...
  arrange(bm) %>%
  mutate(
    caseid_16 = ifelse(is.na(caseid_nov), orig_id, caseid_dec),
...
dat5 <- left_join(dat4,
  ...
  arrange(bm) %>%
  mutate(
    caseid_15 = ifelse(is.na(caseid_dec) & is.na(caseid_15) & survey != 7, orig_id, caseid_nov),
    caseid_nov = ifelse(is.na(caseid_nov), caseid_15, caseid_nov),
...
dat5 %>% filter(survey == 7)
# These should all be 4 as all but the _dat4 column should be null
stopifnot(all(dat5 %>% filter(survey == 7) %>% select(starts_with("caseid")) %>% is.na() %>% rowSums() == 4))
...
dat5 <- dat5 %>%
  mutate() %>%
  select(-c(caseid_16_csv, caseid_16_dat4, caseid_15))
...
# Sanity check: Everyone from survey 8 should have an orig_id matching caseid_dec
# Similarly, from survey 5 should have an orig_id matching caseid_nov
all(dat5$orig_id[dat5$survey == 8] == dat5$caseid_dec[dat5$survey == 8])
all(dat5$orig_id[dat5$survey == 5] == dat5$caseid_nov[dat5$survey == 5])
...
stopifnot(all(apply(dat5[dat5$survey != 7, c("caseid_nov", "caseid_dec"), ], 1, function(x) {
  any(!is.na(x))
})))

test_dat5_dat <- dat5 %>%
  filter(some_match & !is.na(caseid_nov)) %>%
  sample_n(10) %>%
...
test_csv_dat <- dec_dat1 %>%
  filter(caseid_15 %in% test_dat5_dat$caseid_nov[!is.na(test_dat5_dat$caseid_nov)] |
    caseid_16 %in% test_dat5_dat$caseid_dec) %>%
  select(caseid_15, caseid_16) %>%
  arrange(caseid_15)
test_csv_dat
...
testdat <- dat5 %>%
  group_by(bm) %>%
  summarize(test = caseid_nov[1] - caseid_nov[2]) %>%
  filter(!is.na(test))
...
within_person_match <- dat5 %>%
  group_by(bm) %>%
  filter(caseid_nov[1] == caseid_nov[2] | caseid_dec[1] == caseid_dec[2])
stopifnot(nrow(within_person_match) == 0)
within_person_match_2 <- dat5 %>%
  group_by(bm) %>%
  filter(orig_id[1] == caseid_nov[2] | orig_id[2] == caseid_nov[1] |
...
# Yes! 37 people appear twice
table(table(dat5$caseid_nov))
table(table(dat5$caseid_dec))

# Note that, while it could be true that 74 = 37 x 2 different pairs contains one of
# our offending doppelgangers, we only find 66 pairs containing any of their ids. That
# is because 8 pairs consist of duplicate ids
x <- table(dat5$caseid_nov)
bad_caseid_novs <- as.numeric(names(x[x == 2]))
table(table(dat5[dat5$caseid_nov %in% bad_caseid_novs, "bm"]))

dat5 %>% filter(caseid_nov == 1264422787)
