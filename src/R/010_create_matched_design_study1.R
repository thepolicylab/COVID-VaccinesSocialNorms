...
do_matching <- function(infile, outfile_match, outfile_study, pair_limits = NULL) {
  dat0 <- read.csv(infile)
  ...
  ## Removing columns that are either constant or that we match on exactly otherwise can't invert matrix also don't match on stuff we are matching on exactly
  ...
  ## Like a propensity distance on perceptions.
  ## Used below as a penalty on the overall mahalanobis distance matrix
  ## Mostly to capture nonlinear relationships.
  library(splines)
  ...
  ## Rescale and round mhdist mat --- helps for some reason, not 100% clear why
  ##  nmatch() really wants low precision.
  ...
  ## Here we restrict the pairing with some contants based on what we take to be substantively meaningful
  ## differences (like no more than X years of age differences) and also
  ## some that restrict the differences based on the distributions of
  ## differences above (using quantiles) --- like don't match people who differ
  ## more than the 90th percentile of such differences on that variable.

  if (is.null(pair_limits)) {
    ## These are the original pair_limits
    pair_limits <- c(
  ...
  save(dat3, tab, pair_diffs, xbres_vars, xbres, file = outfile_match)
  save(dat0, dat1, dat2, dat3, file = outfile_study)
}
