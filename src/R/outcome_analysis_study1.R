...
lm(q60_1a ~ 1, fixed_effects = ~bm, data = dat3)
lm(q60_2a ~ 1, fixed_effects = ~bm, data = dat3)
lm(q60_3a ~ 1, fixed_effects = ~bm, data = dat3)
lm(q60_4a ~ 1, fixed_effects = ~bm, data = dat3)
...
coherence <- function(data, block = NULL) {
  ...
  ## - say we have 4 outcomes, count number of times obs i has outcomes lower than each other observation. (say, 0 times lower, or only one out of the 4 lower, or all four lower).
  ## - count the number of times that an observation has all four below the other obs (i.e. is the lowest ranked of all of the other obs).
  ## - also count the number of times that an observation has all four above the other obs.
...
save(dat3, xbres, lm_mask_fam, lm_test_fam, lm_perc_fam, lm_perc_neigh, lm_perc_city, lm_perc_state, xb1, mvtest1, mvtest2, mvtest3, mvtest4,
  ...
  file = here::here(MATCHES_DIR, "outcome_analysis_study1.rda")
)
